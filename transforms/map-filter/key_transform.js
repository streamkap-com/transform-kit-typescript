...
// Function: key_transform
// Generated on: 2025-08-26T11:55:34.960Z
// 
// Implementation details:
// - Transform type: map_filter
// - Language: JAVASCRIPT
// - Function type: key_transform
...
var StreamkapTransforms = (() => {
  ...
  __export(templates_exports, {
    ...
    KeySchemaTransform: () => KeySchemaTransform,
    KeyTransform: () => KeyTransform,
    TopicTransform: () => TopicTransform,
    ValueSchemaTransform: () => ValueSchemaTransform,
    ValueTransform: () => ValueTransform,
    commonTransform: () => commonTransform,
    initializeTransforms: () => initializeTransforms,
    keySchemaTransform: () => keySchemaTransform,
    keyTransform: () => keyTransform,
    topicTransform: () => topicTransform,
    valueSchemaTransform: () => valueSchemaTransform,
  ...
  var CommonTransform = class {
    transformRecord(inputRecord, timestamp) {
      ...
      const hasValidData = Boolean(cleanedRecord.id && !import_lodash.default.isEmpty(cleanedRecord.id) || cleanedRecord._id && !import_lodash.default.isEmpty(cleanedRecord._id) || cleanedRecord.order_id && !import_lodash.default.isEmpty(cleanedRecord.order_id));
    ...
    removeUndefinedValues(obj, seen = new WeakSet()) {
      ...
      if (Array.isArray(obj)) {
        const result = obj.map((item) => this.removeUndefinedValues(item, seen)).filter((item) => item !== void 0);
        seen.delete(obj);
        return result;
      }
      const cleaned = {};
  ...
  var KeyTransform = class {
    transform(valueObject, keyObject, topic, timestamp) {
      try {
        ...
        const datePrefix = import_moment2.default.utc(normalizedTimestamp).format("YYYY-MM-DD");
        if (valueObject.user_id) {
          ...
          const sanitizedResult2 = this.sanitizeKey(result2);
          if (!this.validateKey(sanitizedResult2)) {
            console.warn("Generated key failed validation:", sanitizedResult2);
            return this.sanitizeKey(`fallback-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`);
          }
          return sanitizedResult2;
        }
    ...
    transformWithContext(valueObject, keyObject, topic, timestamp) {
      try {
        ...
        if (valueObject.tenant_id && typeof valueObject.tenant_id === "string") {
          ...
          const datePrefix = import_moment2.default.utc(normalizedTimestamp).format("YYYY-MM");
  ...
  var KeySchemaTransform = class {
    ...
    transformToOptimized(valueObject, keyObject, topic, timestamp) {
      try {
        ...
        optimizedKey.ts = Math.floor(safeTimestamp / 1e3);
        if (keyObject.key) {
